...
export default function GradientEdge({ id, sourceX, sourceY, targetX, targetY, sourcePosition, targetPosition, style = {}, data, selected }) {
  ...
  // 👇 Recibimos la orden de exportación desde App.jsx 👇
  const isExportingLight = data?.isExportingLight || false;
  ...
  // 👇 NUEVO: Si la flecha se deselecciona, cerramos sus pestañas 👇
  useEffect(() => {
    if (!selected) {
      setActiveTab(null);
    }
  ...
  // Si estamos exportando, apagamos el fondo negro de las puntas huecas
  const openFillColor = isExportingLight ? 'transparent' : '#0B0F17';
  ...
  return (
    <>
      <defs>
        ...
        <marker id={`circle-open-${id}`} viewBox="0 0 10 10" refX="5" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse"><circle cx="5" cy="5" r="3" fill={openFillColor} stroke={color} strokeWidth="2" /></marker>
        ...
        <marker id={`square-open-${id}`} viewBox="0 0 10 10" refX="5" refY="5" markerWidth="5" markerHeight="5" orient="auto-start-reverse"><rect x="2" y="2" width="6" height="6" fill={openFillColor} stroke={color} strokeWidth="2" /></marker>
        ...
        <marker id={`diamond-open-${id}`} viewBox="0 0 10 10" refX="5" refY="5" markerWidth="6" markerHeight="6" orient="auto-start-reverse"><polygon points="5,1 9,5 5,9 1,5" fill={openFillColor} stroke={color} strokeWidth="2" /></marker>
      ...
      <BaseEdge
        ...
        style={{
          ...
          // 👇 Si estamos exportando, APAGAMOS el resplandor 👇
          filter: isExportingLight ? 'none' : `drop-shadow(0px 0px 4px ${color}) drop-shadow(0px 0px 1px rgba(255,255,255,0.4))`,
          transition: 'stroke 0s, filter 0s' // Transición instantánea
        }}
      />

      {/* ... MENÚ ESTILO CANVA (sin cambios) ... */}
      {selected && createPortal(
        <div onClick={(e) => e.stopPropagation()} onDoubleClick={(e) => e.stopPropagation()} style={{ position: 'fixed', bottom: '32px', left: '50%', transform: 'translateX(-50%)', pointerEvents: 'all', zIndex: 99999 }} className="nodrag nopan flex flex-col items-center">
          {activeTab && (
            <div className="mb-2 bg-[#141923] border border-slate-700 p-2.5 rounded-xl shadow-[0_10px_30px_rgba(0,0,0,0.5)] flex items-center gap-2 animate-in slide-in-from-bottom-2 fade-in duration-200">
              ...
              {activeTab === 'startMarker' && (
                <div className="flex flex-col gap-1 bg-[#0B0F17] p-1 border border-slate-800 rounded">
                  <div className="flex gap-1">{markerRow1.map(type => (<button key={type} onClick={() => updateEdge({ startMarker: type })} className={`p-1.5 rounded transition-colors ${startMarkerType === type ? 'bg-slate-700 text-white' : 'text-slate-500 hover:text-white'}`}><div className="rotate-180"><MarkerIcon type={type} /></div></button>))}</div>
                  <div className="flex gap-1">{markerRow2.map(type => (<button key={type} onClick={() => updateEdge({ startMarker: type })} className={`p-1.5 rounded transition-colors ${startMarkerType === type ? 'bg-slate-700 text-white' : 'text-slate-500 hover:text-white'}`}><div className="rotate-180"><MarkerIcon type={type} /></div></button>))}</div>
                </div>
              )}

              {activeTab === 'endMarker' && (
                <div className="flex flex-col gap-1 bg-[#0B0F17] p-1 border border-slate-800 rounded">
                  <div className="flex gap-1">{markerRow1.map(type => (<button key={type} onClick={() => updateEdge({ endMarker: type })} className={`p-1.5 rounded transition-colors ${endMarkerType === type ? 'bg-slate-700 text-white' : 'text-slate-500 hover:text-white'}`}><MarkerIcon type={type} /></button>))}</div>
                  <div className="flex gap-1">{markerRow2.map(type => (<button key={type} onClick={() => updateEdge({ endMarker: type })} className={`p-1.5 rounded transition-colors ${endMarkerType === type ? 'bg-slate-700 text-white' : 'text-slate-500 hover:text-white'}`}><MarkerIcon type={type} /></button>))}</div>
                </div>
              )}
          ...
          <div className="bg-[#0B0F17]/90 backdrop-blur-md border border-slate-700/80 p-1.5 rounded-xl shadow-[0_20px_40px_rgba(0,0,0,0.6)] flex items-center gap-1">
            <button onClick={() => setActiveTab(activeTab === 'color' ? null : 'color')} className={`p-1.5 rounded-md hover:bg-slate-800 transition-colors ${activeTab === 'color' ? 'bg-slate-800' : ''}`} title="Color"><div className="w-4 h-4 rounded-full border border-slate-600" style={{ backgroundColor: color, boxShadow: `0 0 4px ${color}` }} /></button>
            <div className="w-px h-5 bg-slate-700 mx-1" />
            <button onClick={() => setActiveTab(activeTab === 'style' ? null : 'style')} className={`p-1.5 rounded-md text-slate-400 hover:text-white hover:bg-slate-800 transition-colors ${activeTab === 'style' ? 'bg-slate-800 text-white' : ''}`} title="Grosor y estilo"><AlignJustify size={16} /></button>
            <div className="w-px h-5 bg-slate-700 mx-1" />
            <button onClick={() => setActiveTab(activeTab === 'startMarker' ? null : 'startMarker')} className={`p-1.5 rounded-md text-slate-400 hover:text-white hover:bg-slate-800 transition-colors ${activeTab === 'startMarker' ? 'bg-slate-800 text-white' : ''}`} title="Inicio de línea"><div className="rotate-180"><MarkerIcon type={startMarkerType} /></div></button>
            <button onClick={() => setActiveTab(activeTab === 'endMarker' ? null : 'endMarker')} className={`p-1.5 rounded-md text-slate-400 hover:text-white hover:bg-slate-800 transition-colors ${activeTab === 'endMarker' ? 'bg-slate-800 text-white' : ''}`} title="Fin de línea"><MarkerIcon type={endMarkerType} /></button>
            <button onClick={swapDirection} className="p-1.5 rounded-md text-indigo-400 hover:text-indigo-300 hover:bg-indigo-500/10 transition-colors" title="Invertir dirección"><ArrowLeftRight size={16} /></button>
            <div className="w-px h-5 bg-slate-700 mx-1" />
            <button onClick={() => setActiveTab(activeTab === 'routing' ? null : 'routing')} className={`p-1.5 rounded-md text-slate-400 hover:text-white hover:bg-slate-800 transition-colors ${activeTab === 'routing' ? 'bg-slate-800 text-white' : ''}`} title="Forma de línea">{routing === 'straight' ? <Slash size={16} /> : routing === 'step' ? <CircuitIcon /> : <Spline size={16} />}</button>
            <div className="w-px h-5 bg-slate-700 mx-1" />
            <button onClick={() => setActiveTab(activeTab === 'transparency' ? null : 'transparency')} className={`p-1.5 px-2 rounded-md text-xs font-bold text-slate-400 hover:text-white hover:bg-slate-800 transition-colors ${activeTab === 'transparency' ? 'bg-slate-800 text-white' : ''}`} title="Transparencia">{(opacity * 100).toFixed(0)}%</button>
          </div>
        </div>,
        document.body
      )}
