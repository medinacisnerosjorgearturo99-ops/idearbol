...
export default function NetworkNode({ data, selected }) {
  ...
  const color = isGroup ? '#10b981' : '#6366f1'; // Colores base

  // Estilo mágico para que los puntos brillen como neón
  const handleStyle = {
  ...
  return (
    <div className={`bg-[#141923] border ${selected ? 'border-slate-300 shadow-[0_0_15px_rgba(255,255,255,0.1)]' : 'border-slate-700/80'} rounded-xl shadow-2xl overflow-hidden min-w-[180px] transition-all`}>
      ...
      <Handle type="target" position={Position.Top} id="top" style={{ ...handleStyle, top: '-5px' }} className="hover:scale-150" />
      ...
      <Handle type="target" position={Position.Left} id="left" style={{ ...handleStyle, left: '-5px' }} className="hover:scale-150" />
    </div>
  );
}